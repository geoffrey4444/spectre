...
void generalized_eigenvalue(const Matrix& A, const Matrix& B,
  ...
  static_assert(number_of_rows == A.columns(), "Matrix A should be square");
  static_assert(number_of_rows == B.rows() && number_of_rows == B.columns(),
                "Matrix A and matrix B should be the same size");
  static_assert(number_of_rows == eigenvectors.rows() &&
                    number_of_rows == eigenvectors.columns(),
                "Matrix A and matrix eigenvectors should have the same size");
  static_assert(number_of_rows == eigenvalues_real_part.size() &&
  ...
  const int matrix_and_vector_size = static_cast<int>(number_of_rows);
  ...
  const size_t work_size = number_of_rows * 8;
  ...
  dggev_(&compute_left_eigenvectors, &compute_right_eigenvectors,
         &matrix_and_vector_size, A.data(), &matrix_and_vector_size, B.data(),
  ...
  static_assert(info == 0, "Lapack failed to compute generalized eigenvectors");
  ...
  for (size_t i = 0; i < eigenvalues_real_part.size(); ++i) {
    eigenvalues_real_part[i] =
        unnormalized_eigenvalue_real_part[i] / eigenvalue_normalization;
    eigenvalues_imaginary_part[i] =
        unnormalized_eigenvalue_imaginary_part[i] / eigenvalue_normalization;
  }
}
}  // namespace
