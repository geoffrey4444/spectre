...
template <bool AllowSettleFoTs>
std::unique_ptr<domain::FunctionsOfTime::FunctionOfTime> set_expansion(
  ...
  if (std::holds_alternative<FromVolumeFile>(expansion_map_options)) {
    ...
    // It must be either a PiecewisePolynomial or a SettleToConstant
    ...
    if (pp_volume_fot != nullptr) {
      result =
          volume_fot.at(name)->create_at_time(initial_time, expiration_time);
    } else {
      ASSERT(AllowSettleFoTs,
             "Expansion function of time in the volume file is a "
             "SettleToConstant, but SettleToConstant functions of time aren't "
             "allowed.");
      result = volume_fot.at(name)->get_clone();
    }
    ...
    if (hard_coded_options.asymptotic_velocity_outer_boundary.has_value()) {
      ...
      result = std::make_unique<domain::FunctionsOfTime::SettleToConstant>(
...
set_expansion_outer_boundary(
    const std::variant<ExpansionMapOptions<AllowSettleFoTs>, FromVolumeFile>&
        expansion_map_options,
    const double initial_time) {
  const std::string name = "ExpansionOuterBoundary";
  std::unique_ptr<domain::FunctionsOfTime::FunctionOfTime> result{};
  ...
  if (std::holds_alternative<FromVolumeFile>(expansion_map_options)) {
    ...
    // It must be either a FixedSpeedCubic or a SettleToConstant
    const auto* cubic_volume_fot =
    ...
    if (UNLIKELY(cubic_volume_fot == nullptr and
                 settle_volume_fot == nullptr)) {
      ERROR_NO_TRACE(
    ...
    ASSERT(cubic_volume_fot != nullptr or AllowSettleFoTs,
    ...
    if (hard_coded_options.asymptotic_velocity_outer_boundary.has_value()) {
      result = std::make_unique<domain::FunctionsOfTime::FixedSpeedCubic>(
          hard_coded_options.initial_values_outer_boundary[0][0], initial_time,
...
#define INSTANTIATE(_, data)                                               \
  ...
  set_expansion(const std::variant<ExpansionMapOptions<ALLOWSETTLE(data)>, \
