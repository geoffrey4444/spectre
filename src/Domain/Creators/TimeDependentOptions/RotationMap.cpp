...
RotationMapOptions<AllowSettleFoTs>::RotationMapOptions(
  ...
  if (initial_angles.empty() or initial_angles.size() > 3) {
    PARSE_ERROR(context,
                "Must specify at least the angle, and optionally up to 2 time "
                "derivatives.");
  }
  for (size_t i = 0; i < initial_angles.size(); i++) {
    gsl::at(angles, i) = DataVector{initial_angles[i]};
  }
}
...
std::unique_ptr<domain::FunctionsOfTime::FunctionOfTime> set_rotation(
  ...
  if (std::holds_alternative<FromVolumeFile>(rotation_map_options)) {
    ...
    if (quat_volume_fot != nullptr) {
      result = quat_volume_fot->create_at_time(initial_time, expiration_time);
    } else {
      ASSERT(
          AllowSettleFoTs,
          "Rotation function of time in the volume file is a SettleToConstant, "
          "but SettleToConstant functions of time aren't allowed.");
...
#define INSTANTIATE(_, data)                                             \
  ...
  set_rotation(const std::variant<RotationMapOptions<ALLOWSETTLE(data)>, \
