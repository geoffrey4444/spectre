 ...
 * \f$z \to z\f$, and the translation is implemented as a product of the 2D
 * rotation and an identity map. This time dependence is the identity for
 * 1 spatial dimension.
 */
...
class UniformRotationAboutZAxis final : public TimeDependence<MeshDim> {
 ...
 public:
  ...
  struct FunctionOfTimeName {
    ...
    static constexpr OptionString help = {"Name of the functions of time."};
  ...
  static constexpr OptionString help = {
      "A spatially uniform rotation initialized with a constant angular "
      "velocity."};
  ...
  UniformRotationAboutZAxis(double initial_time, double angular_velocity,
                            std::string function_of_time_name = std::string{
                                "RotationAngle"}) noexcept;
