...
struct Shape : tt::ConformsTo<protocols::ControlError> {
  ...
  DataVector operator()(const ::TimescaleTuner<true>& /*unused*/,
    ...
    const ylm::Spherepack lambda_spherepack = [](const size_t spectral_size) {
      ...
      const auto l_max_plus_one =
          static_cast<size_t>(sqrt(static_cast<double>(spectral_size) / 2.0));
      ...
      return ylm::Spherepack{l_max, l_max};
    }(lambda_lm_coefs.size());
    ...
    DataVector resized_ah_coefs{};
    std::reference_wrapper<const DataVector> ah_coefs_for_error =
        std::cref(ah_coefs);
    if (lambda_lm_coefs.size() != ah_coefs.size()) {
      resized_ah_coefs =
          ah.ylm_spherepack().prolong_or_restrict(ah_coefs, lambda_spherepack);
      ah_coefs_for_error = std::cref(resized_ah_coefs);
    }
    const DataVector& ah_coefs_ref = ah_coefs_for_error.get();
    ...
    ylm::SpherepackIterator iter{lambda_spherepack.l_max(),
                                 lambda_spherepack.m_max()};
