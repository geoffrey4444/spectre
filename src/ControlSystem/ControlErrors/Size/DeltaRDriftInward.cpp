...
bool should_transition_from_state_delta_r_to_inward_drift(
    const CrossingTimeInfo& crossing_time_info, const double damping_time,
  ...
  const bool radial_distance_requires_inward_drift =
      update_args.min_allowed_radial_distance.has_value() and
      update_args.minimum_radial_distance.has_value() and
      update_args.minimum_radial_distance.value() <=
          inward_drift_limit_buffer_factor *
              update_args.min_allowed_radial_distance.value() and
      not(crossing_time_info.t_drift_limit_delta_radius.has_value() and
          crossing_time_info.t_drift_limit_delta_radius.value() < damping_time);
  const bool char_speed_requires_inward_drift =
      update_args.min_allowed_char_speed.has_value() and
      update_args.min_char_speed <=
          inward_drift_limit_buffer_factor *
              update_args.min_allowed_char_speed.value() and
...
bool should_transition_from_state_inward_drift_to_delta_r_no_drift(
  ...
  return (not should_transition_from_state_delta_r_to_inward_drift(
      crossing_time_info, damping_time, update_args));
}
...
bool ok_to_return_to_state_deltar(const StateUpdateArgs& update_args) {
  ...
  const bool has_radial_distance_limit =
      update_args.min_allowed_radial_distance.has_value();
  const bool has_char_speed_limit =
      update_args.min_allowed_char_speed.has_value();
  const bool delta_r_large_enough_to_stop_inward_drift =
      not has_radial_distance_limit or
      (update_args.minimum_radial_distance.has_value() and
       update_args.minimum_radial_distance.value() >
           stop_inward_drift_buffer_factor *
               update_args.min_allowed_radial_distance.value());
  const bool char_speed_large_enough_to_stop_inward_drift =
      not has_char_speed_limit or
      update_args.min_char_speed >
          stop_inward_drift_buffer_factor *
              update_args.min_allowed_char_speed.value();
  return (has_radial_distance_limit or has_char_speed_limit) and
         delta_r_large_enough_to_stop_inward_drift and
