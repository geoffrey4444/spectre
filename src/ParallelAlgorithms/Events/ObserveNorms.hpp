 ...
 * for the L2 norm we have (for a vector)
 *
 * \f{align*}{
 * L_2(v^i)=\sqrt{\frac{1}{N}\sum_{i=0}^{N} \left[(v^x_i)^2 + (v^y_i)^2
 *          + (v^z_i)^2\right]}
 * \f}
 */
...
class ObserveNorms : public Event {
 private:
  struct ObserveTensor {
    static constexpr Options::String help = {"Blah"};

    struct Name {
      using type = std::string;
      static constexpr Options::String help = {"The of the tensor to observe."};
    };
    struct NormType {
      using type = std::string;
    ...
    struct Components {
      using type = std::string;
      static constexpr Options::String help = {
          "How to handle components. Must be Individual or Sum."};
    };
 ...
 public:
  ...
  ObserveNorms(const std::string& subfile_name,
               std::vector<ObserveTensor> observe_tensors) noexcept;
...
/// \cond
template <typename... ObservableTensorTags>
ObserveNorms<ObservableTensorTags...>::ObserveNorms(
...
template <typename... ObservableTensorTags>
ObserveNorms<ObservableTensorTags...>::ObserveTensor::ObserveTensor(
  ...
  if (components != "Individual" and components != "Sum") {
    PARSE_ERROR(context,
                "Components must Individual or Sum, not " << components);
  }
}
...
template <typename DbTagsList, typename Metavariables, typename ArrayIndex,
          typename ParallelComponent>
void ObserveNorms<ObservableTensorTags...>::operator()(
  ...
  tmpl::for_each<tensor_tags>([this, &box, &norm_values_and_names,
    ...
    for (size_t i = 0; i < tensor_names_.size(); ++i) {
      if (tensor_name == tensor_names_[i]) {
        auto& values_and_names = norm_values_and_names[tensor_norm_types_[i]];
        ...
        if (tensor_components_[i] == "Individual") {
          for (size_t storage_index = 0; storage_index < component_names.size();
            ...
            if (tensor_norm_types_[i] == "Max") {
              values_and_names.first.push_back(max(components[storage_index]));
            } else if (tensor_norm_types_[i] == "Min") {
              values_and_names.first.push_back(min(components[storage_index]));
            } else if (tensor_norm_types_[i] == "L2Norm") {
              values_and_names.first.push_back(
                  alg::accumulate(square(components[storage_index]), 0.0));
            }
            values_and_names.second.push_back(
          ...
          values_and_names.second.push_back(tensor_norm_types_[i] + "(" +
                                            tensor_name + ")");
          values_and_names.first.push_back(value);
        }
      }
    }
  });
...
template <typename... ObservableTensorTags>
void ObserveNorms<ObservableTensorTags...>::pup(PUP::er& p) {
  Event::pup(p);
  p | subfile_path_;
  p | tensor_names_;
...
template <typename... ObservableTensorTags>
PUP::able::PUP_ID ObserveNorms<ObservableTensorTags...>::my_PUP_ID =
    0;  // NOLINT
/// \endcond
}  // namespace Events
