 ...
 * `ah::Tags::MaxResolutionAndOutputL`. The value returned for $L_{\rm new}$ is
...
size_t Residual::operator()(const Parallel::GlobalCache<Metavariables>& cache,
                            const ylm::Strahlkorper<Frame>& strahlkorper,
                            const FastFlow::IterInfo& info) const {
  const auto& max_resolution_l =
      Parallel::get<ah::Tags::MaxResolutionAndOutputL>(cache);
  ASSERT(min_resolution_l_ <= max_resolution_l,
         "MinResolutionL (" << min_resolution_l_
                            << ") must not exceed MaxResolutionAndOutputL ("
                            << max_resolution_l << ").");
  if (UNLIKELY(min_resolution_l_ == max_resolution_l)) {
    ASSERT(min_resolution_l_ == strahlkorper.l_max(),
           "If MinResolutionL == MaxResolutionAndOutputL, strahlkorper "
           "resolution must also equal MinResolutionL, but here MinResolutionL "
           "is "
               << min_resolution_l_ << ", MaxResolutionAndOutputL is "
               << max_resolution_l << ", and the current resolution is "
               << strahlkorper.l_max());
