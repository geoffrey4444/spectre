...
struct KerrSchildTeukolskyVariables
  ...
  using kerr_schild_tags =
      tmpl::list<gr::Tags::SpatialMetric<DataType, Dim>,
                 ...
                 ::Tags::deriv<gr::Tags::Shift<DataType, Dim>,
                               tmpl::size_t<Dim>, Frame::Inertial>,
  ...
  const tuples::tagged_tuple_from_typelist<kerr_schild_tags>& kerr_schild_vars;
  const tuples::tagged_tuple_from_typelist<teukolsky_tags>& teukolsky_vars;
  ...
  void operator()(
      ...
      gsl::not_null<Cache*> cache,
      gr::Tags::ExtrinsicCurvature<DataType, Dim> /*meta*/) const override;
  void operator()(
  ...
  void operator()(gsl::not_null<tnsr::I<DataType, Dim>*> spatial_velocity,
                  gsl::not_null<Cache*> cache,
  ...
  void operator()(gsl::not_null<tnsr::I<DataType, Dim>*> magnetic_field,
                  gsl::not_null<Cache*> cache,
...
/*!
 * \brief Kerr-Schild XCTS data with a Teukolsky perturbation in the conformal
 * metric.
 *
 * The free data follows the SpEC setup where a regular Cartesian Kerr-Schild
 * black hole supplies the lapse, shift, and trace of the extrinsic curvature,
 * while an even-parity ingoing Teukolsky wave at \f$t=0\f$ is added only to the
 * XCTS conformal metric.
 */
class KerrSchildTeukolsky : public elliptic::analytic_data::AnalyticSolution {
 public:
  static constexpr size_t Dim = 3;

  class TeukolskyWaveOptions {
   public:
    struct Amplitude {
      using type = double;
    ...
    struct Width {
      using type = double;
      static constexpr Options::String help{"Width of the Gaussian pulse"};
      static type lower_bound() { return 0.0; }
    };

    using options = tmpl::list<Amplitude, Mode, Radius, Width>;
    static constexpr Options::String help{
        "Even-parity ingoing Teukolsky perturbation at t=0"};

    TeukolskyWaveOptions() = default;
    TeukolskyWaveOptions(double amplitude, int mode, double radius,
                         double width, const Options::Context& context = {});
  ...
  struct TeukolskyWave {
    using type = TeukolskyWaveOptions;
    static constexpr Options::String help{
  ...
  std::unique_ptr<elliptic::analytic_data::AnalyticSolution> get_clone()
      const override {
    return std::make_unique<KerrSchildTeukolsky>(*this);
  }
  /// \endcond
