...
/// Reduced constraint maxima keyed by `(time, slab_number)`.
struct ConstraintCheckRecords : db::SimpleTag {
  using key_type = std::pair<double, int64_t>;
...
struct BroadcastCompletionRequestToElements<
    Metavariables, std::void_t<typename Metavariables::gh_dg_element_array>> {
  static void apply(Parallel::GlobalCache<Metavariables>& cache) {
...
void recompute_completion_state(const gsl::not_null<db::DataBox<DbTags>*> box,
  ...
  std::vector<HorizonSample> horizon_samples{};
  const auto& horizon_records =
      db::get<gh::bbh::Tags::CommonHorizonSuccessRecords>(*box);
  horizon_samples.reserve(horizon_records.size());
  for (const auto& [time_id, l_max] : horizon_records) {
    horizon_samples.push_back(HorizonSample{time_id.id, l_max});
  }
  std::sort(horizon_samples.begin(), horizon_samples.end(),
            [](const HorizonSample& a, const HorizonSample& b) {
              return a.time < b.time;
            });

  const size_t success_count = horizon_samples.size();
  bool lmax_latched = false;
  std::optional<HorizonSample> first_lmax_match{};
  for (const auto& sample : horizon_samples) {
    if (sample.l_max <= l_max_threshold) {
      lmax_latched = true;
      first_lmax_match = sample;
      break;
    }
  }

  bool gauge_exceeded = false;
  bool three_index_exceeded = false;
  std::optional<double> first_gauge_match_time{};
  std::optional<double> first_three_index_match_time{};
  size_t horizon_index = 0;
  ...
  for (const auto& [constraint_id, maxima] : constraint_records) {
    const double constraint_time = constraint_id.first;
    while (horizon_index < horizon_samples.size() and
           horizon_samples[horizon_index].time <= constraint_time) {
    ...
    if (verbose and not reported_constraint_records.contains(constraint_id)) {
      Parallel::printf(
      ...
      newly_reported_constraint_records.push_back(constraint_id);
    }
    if (not gauge_exceeded and maxima.first >= gauge_constraint_threshold) {
      gauge_exceeded = true;
      first_gauge_match_time = constraint_time;
    }
    if (not three_index_exceeded and
        maxima.second >= three_index_constraint_threshold) {
      three_index_exceeded = true;
  ...
  const bool completion_requested =
      horizon_completion_requested or gauge_exceeded or three_index_exceeded;
  ...
  db::mutate<gh::bbh::Tags::GaugeConstraintExceeded,
      ...
      [&gauge_exceeded, &three_index_exceeded, &lmax_latched, &success_count,
        ...
        *gauge_constraint_exceeded_flag = gauge_exceeded;
        *three_index_constraint_exceeded_flag = three_index_exceeded;
  ...
  if (not old_lmax_latched and lmax_latched and first_lmax_match.has_value()) {
    Parallel::printf(
        ...
        first_lmax_match->time, first_lmax_match->l_max, l_max_threshold);
  }
  if (not old_gauge_exceeded and gauge_exceeded and
      first_gauge_match_time.has_value()) {
    Parallel::printf(
        "BBH completion criterion met at t=%.16f: "
        "Linf(GaugeConstraint) >= %.16e.\n",
        *first_gauge_match_time, gauge_constraint_threshold);
  }
  if (not old_three_index_exceeded and three_index_exceeded and
...
struct RecordCommonHorizonSuccess {
  ...
  static void apply(db::DataBox<DbTags>& box,
    ...
    const auto [it, inserted] =
        common_horizon_success_records.emplace(temporal_id, l_max);
    if (not inserted) {
      it->second = std::min(it->second, l_max);
    }
  ...
  static void apply(db::DataBox<DbTags>& box,
    ...
    auto& constraint_check_records =
        db::get_mutable_reference<gh::bbh::Tags::ConstraintCheckRecords>(
            make_not_null(&box));
    const gh::bbh::Tags::ConstraintCheckRecords::key_type key{time,
                                                              slab_number};
    const auto [it, inserted] = constraint_check_records.emplace(
        key, gh::bbh::Tags::ConstraintCheckRecords::mapped_type{
                 max_gauge_linf, max_three_index_linf});
    if (not inserted) {
      it->second.first = std::max(it->second.first, max_gauge_linf);
      it->second.second = std::max(it->second.second, max_three_index_linf);
    }
    detail::recompute_completion_state(make_not_null(&box), cache);
  }
};
