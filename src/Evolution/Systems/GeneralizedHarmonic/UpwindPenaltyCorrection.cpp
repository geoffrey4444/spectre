...
void UpwindPenaltyCorrection<Dim>::package_data(
    ...
    const gsl::not_null<tnsr::aa<DataVector, Dim, Frame::Inertial>*>
        packaged_gamma2_v_spacetime_metric,
  ...
  for (size_t storage_index = 0; storage_index < v_spacetime_metric.size();
       ++storage_index) {
    packaged_v_spacetime_metric->operator[](storage_index) =
        char_speeds[0] * v_spacetime_metric[storage_index];
    packaged_gamma2_v_spacetime_metric->operator[](storage_index) =
  ...
  *packaged_v_zero = v_zero;
  for (size_t storage_index = 0; storage_index < v_zero.size();
       ++storage_index) {
    packaged_v_zero->operator[](storage_index) *= char_speeds[1];
  }

  for (size_t storage_index = 0; storage_index < v_plus.size();
       ++storage_index) {
    packaged_v_plus->operator[](storage_index) =
        char_speeds[2] * v_plus[storage_index];
    packaged_v_minus->operator[](storage_index) =
  ...
  for (size_t a = 0; a < Dim + 1; ++a) {
    for (size_t b = 0; b <= a; ++b) {
      for (size_t i = 0; i < Dim; ++i) {
        packaged_n_times_v_plus->get(i, a, b) =
            packaged_v_plus->get(a, b) * interface_unit_normal.get(i);
        packaged_n_times_v_minus->get(i, a, b) =
            packaged_v_minus->get(a, b) * interface_unit_normal.get(i);
      }
    }
  }
  for (size_t i = 0; i < 4; ++i) {
    (*packaged_char_speeds)[i] = char_speeds[i];
  }
}
...
void UpwindPenaltyCorrection<Dim>::operator()(
    ...
    const tnsr::iaa<DataVector, Dim, Frame::Inertial>& normal_times_v_plus_int,
    const tnsr::iaa<DataVector, Dim, Frame::Inertial>& normal_times_v_minus_int,
    const tnsr::aa<DataVector, Dim, Frame::Inertial>&
        constraint_gamma2_v_spacetime_metric_int,
    ...
    const tnsr::aa<DataVector, Dim, Frame::Inertial>&
        constraint_gamma2_v_spacetime_metric_ext,
    const tnsr::a<DataVector, 3, Frame::Inertial>& char_speeds_ext) const
    noexcept {
  ...
  for (size_t storage_index = 0; storage_index < v_spacetime_metric_ext.size();
       ++storage_index) {
    spacetime_metric_boundary_correction->operator[](storage_index) =
        weighted_lambda_spacetime_metric_ext *
            v_spacetime_metric_ext[storage_index] -
        weighted_lambda_spacetime_metric_int *
            v_spacetime_metric_int[storage_index];
  }

  for (size_t storage_index = 0; storage_index < v_spacetime_metric_ext.size();
       ++storage_index) {
    (*pi_boundary_correction)[storage_index] =
        0.5 * (weighted_lambda_plus_ext * v_plus_ext[storage_index] +
               weighted_lambda_minus_ext * v_minus_ext[storage_index]) +
        weighted_lambda_spacetime_metric_ext *
            constraint_gamma2_v_spacetime_metric_ext[storage_index]

        - 0.5 * (weighted_lambda_plus_int * v_plus_int[storage_index] +
                 weighted_lambda_minus_int * v_minus_int[storage_index]) -
        weighted_lambda_spacetime_metric_int *
            constraint_gamma2_v_spacetime_metric_int[storage_index];
  }
  ...
  for (size_t storage_index = 0;
    ...
    (*phi_boundary_correction)[storage_index] =
        -0.5 * (weighted_lambda_minus_ext *
                    minus_normal_times_v_minus_ext[storage_index] -
                weighted_lambda_plus_ext *
                    minus_normal_times_v_plus_ext[storage_index]) +
        weighted_lambda_zero_ext * v_zero_ext[storage_index]

        - 0.5 * (weighted_lambda_plus_int *
                     normal_times_v_plus_int[storage_index] -
                 weighted_lambda_minus_int *
                     normal_times_v_minus_int[storage_index]) -
        weighted_lambda_zero_int * v_zero_int[storage_index];
  }
}
