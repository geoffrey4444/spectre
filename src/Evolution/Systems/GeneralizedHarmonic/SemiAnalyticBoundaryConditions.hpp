...
/// Dirichlet boundary conditions on the exterior side UNLESS
/// i) the system is nonconservative, and ii) all characteristic speeds are
/// outgoing. If both i) and ii) are satisfied, then instead do nothing.
///
/// With:
/// - External<Tag> =
...
///   - Tags::CharacteristicSpeeds<DataVector>
///
/// DataBox changes:
/// - Adds: nothing
/// - Removes: nothing
...
struct ImposeDirichletBoundaryConditionsUnlessOnlyOutgoingCharSpeeds {
 private:
    ...
    db::mutate_apply<tmpl::list<domain::Tags::Interface<
        ...
        [](const gsl::not_null<db::item_type<domain::Tags::Interface<
          ...
          for (auto& external_direction_and_vars : *external_bdry_vars) {
            ...
            auto char_speeds = boundary_char_speeds.at(direction);
