 ...
 * implemented since that boundary term does not consistently treat both sides
 * of the interface on the same footing.
 *
 * For the first-order GH system the correction is:
 *
 * \f{align}{
 ...
 * \f$\pm\f$ fields and speeds reverse roles.
 */
template <size_t Dim>
 ...
 public:
  ...
  using package_field_tags =
      tmpl::list<Tags::VSpacetimeMetric<Dim, Frame::Inertial>,
                 ...
                 NormalTimesVMinus, Gamma2VSpacetimeMetric, TensorCharSpeeds>;
  ...
  void operator()(
      ...
      const tnsr::iaa<DataVector, Dim, Frame::Inertial>&
          normal_times_v_plus_int,
      const tnsr::iaa<DataVector, Dim, Frame::Inertial>&
          normal_times_v_minus_int,
