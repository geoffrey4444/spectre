...
def compute_ringdown_shape_coefs(
    ...
    """Compute and write to disk common horizon coefs in the
    ringdown distorted frame. These coefficients are needed to start a
    ringdown after a common horizon has formed."""
    logger.warning(
        """This code is still experimental. It has been tested for a q=1, spin=0
        quasicircular inspiral but does not yet support accounting for a nonzero
        translation map in the inspiral (necessary for unequal-mass mergers.)"""
    )
...
@click.command(
    ...
    help=compute_ringdown_shape_coefs.__doc__,
)
@click.option(
    "-A",
    "--path_to_ah_h5",
...
def compute_ringdown_shape_coefs_command(**kwargs):
    _rich_traceback_guard = True  # Hide traceback until here
    compute_ringdown_shape_coefs(**kwargs)


if __name__ == "__main__":
    compute_ringdown_shape_coefs_command(help_option_names=["-h", "--help"])
