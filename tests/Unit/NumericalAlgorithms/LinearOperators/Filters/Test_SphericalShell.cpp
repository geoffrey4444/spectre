...
void test_apply_on_boundary() {
  INFO("apply_on_boundary is a no-op");
  // The boundary slice is a 2-sphere with no radial direction, and the
  // angular Ylm filter is applied only in the volume; SphericalShell's
  // boundary hook is intentionally a no-op.
  const Mesh<2> face_mesh{
      {{4, 5}}, Spectral::Basis::Legendre, Spectral::Quadrature::GaussLobatto};
  Variables<TagList> face_vars(face_mesh.number_of_grid_points());
  for (size_t i = 0; i < face_vars.number_of_grid_points(); ++i) {
    const auto i_d = static_cast<double>(i);
    get(get<::CurvedScalarWave::Tags::Psi>(face_vars))[i] = 0.5 + (0.25 * i_d);
    get(get<::CurvedScalarWave::Tags::Pi>(face_vars))[i] = -0.1 + i_d;
    auto& phi =
        get<::CurvedScalarWave::Tags::Phi<3, Frame::Inertial>>(face_vars);
    for (size_t d = 0; d < 3; ++d) {
      phi.get(d)[i] = static_cast<double>(d) - (0.3 * i_d);
    }
  }
  const auto initial = face_vars;

  const auto filter = make_filter(2, 16, 8, true, std::nullopt, false, false,
                                  std::nullopt, std::nullopt);
  filter.apply_on_boundary(make_not_null(&face_vars), face_mesh, std::nullopt,
                           std::nullopt);
  CHECK(face_vars == initial);
}
