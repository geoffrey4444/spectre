...
void test_interpolation_target_receive_vars() noexcept {
  ...
  if constexpr (IsTimeDependent::value) {
    ...
    current_temporal_ids.insert(current_temporal_ids.end(), first_temporal_id);
    pending_temporal_ids.insert(pending_temporal_ids.end(), second_temporal_id);
    ...
    runner_ptr.reset(new ActionTesting::MockRuntimeSystem<metavars>{
