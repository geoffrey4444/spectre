...
struct MockMetavariables {
  using component_list =
      tmpl::list<MockComponent<MockMetavariables, HorizonMetavars<Fr>>>;
  using const_global_cache_tags =
      tmpl::list<domain::Tags::Domain<3>,
                 ah::Tags::ApparentHorizonOptions<HorizonMetavars<Fr>>>;
...
template <typename Fr, bool MakeHorizonFinderFailOnPurpose = false>
void test_apparent_horizon(
  ...
  using metavars = MockMetavariables<Fr>;
  using component = MockComponent<metavars, HorizonMetavars<Fr>>;

  // Assert the protocols
  static_assert(tt::assert_conforms_to_v<HorizonMetavars<Fr>,
                                         ah::protocols::HorizonMetavars>);
  static_assert(
      check_callbacks_v<typename HorizonMetavars<Fr>::horizon_find_callbacks>);
  static_assert(check_callbacks_v<
                typename HorizonMetavars<Fr>::horizon_find_failure_callbacks>);
  ...
  for (const auto& time : times) {
    for (const auto& element_id : element_ids) {
      ...
      ActionTesting::queue_simple_action<
          component, ah::ReceiveVolumeData<HorizonMetavars<Fr>>>(
...
SPECTRE_TEST_CASE("Unit.ApparentHorizonFinder.ReceiveVolumeData",
  ...
  // Failure tests
  test_apparent_horizon<Frame::Inertial, true>(3, 3, 1.0, {{0.0, 0.0, 0.0}},
                                               true);
  ...
  test_apparent_horizon<Frame::Grid, true>(3, 3, 10.0, {{0.0, 0.0, 0.0}},
                                           false);
  CHECK(callback_count == 0);
  CHECK(callback_failure_count == 6);
  CHECK(callback_failure_mode == FastFlow::Status::InterpolationFailure);
