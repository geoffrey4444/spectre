...
void test() {
  ...
  CHECK(criterion_one == criterion_one_same);
  CHECK(criterion_one != criterion_two);
  const auto criterion_one_serialized =
      serialize_and_deserialize(criterion_one);
  CHECK(criterion_one == criterion_one_serialized);
  ...
  CHECK(*criterion_four == *criterion_five);
  CHECK(*criterion_four != *criterion_six);
}
}  // namespace
