...
void test_set_current_time() {
  ...
  // Current time has value, so do nothing
  {
    set_current_time(make_not_null(&current_time),
                     make_not_null(&pending_times), completed_times, verbosity,
                     name);
    CHECK(current_time ==
  ...
  // No completed ids and the pending id isn't the first, so do nothing
  {
  ...
  // One completed id, but the pending id isn't next so do nothing
  {
    current_time.reset();
    pending_times.clear();
