...
void test(const std::optional<bool> use_non_zero_shape) {
  ...
  }
  {
    ...
    check_map(time_dep_options.grid_to_distorted_map(1, 0, false),
              not use_non_zero_shape.has_value(), false);
    check_map(time_dep_options.distorted_to_inertial_map(1, false),
              not use_non_zero_shape.has_value(), false);
    check_map(time_dep_options.grid_to_inertial_map(1, 0, false, false), false,
              false);
    // Shell without shape
    check_map(time_dep_options.grid_to_distorted_map(2, 0, false), true, false);
    check_map(time_dep_options.distorted_to_inertial_map(2, false), true,
              false);
    check_map(time_dep_options.grid_to_inertial_map(2, 0, false, false), false,
              false);
    // Inner cube
    check_map(time_dep_options.grid_to_distorted_map(2, 0, true), true, false);
    check_map(time_dep_options.distorted_to_inertial_map(2, true), true, false);
    check_map(time_dep_options.grid_to_inertial_map(2, 0, false, true), false,
              false);
  }
