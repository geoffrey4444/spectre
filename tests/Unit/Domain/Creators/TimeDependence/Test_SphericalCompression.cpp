...
void test(const std::unique_ptr<TimeDependence<MeshDim>>& time_dep_unique_ptr,
  ...
  const auto* const time_dep =
      dynamic_cast<const SphericalCompression<MeshDim>*>(
  ...
  const auto expected_block_map =
      create_coord_map<MeshDim>(f_of_t_name, min_radius, max_radius, center);
  ...
  // The points are all positive to guarantee that they are in the region where
  // the map is valid (min_radius <= radius <= max_radius)
  TIME_DEPENDENCE_GENERATE_COORDS(make_not_null(&gen), MeshDim,
                                  min_radius * 1.001, max_radius * 0.5);
...
SPECTRE_TEST_CASE("Unit.Domain.Creators.TimeDependence.SphericalCompression",
  ...
  constexpr double initial_value{3.5};
  constexpr double initial_velocity{-4.6};
  constexpr double initial_acceleration{5.7};
  const std::string f_of_t_name{"LambdaFactorA0"};

  const std::unique_ptr<domain::creators::time_dependence::TimeDependence<3>>
      time_dep = std::make_unique<SphericalCompression<3>>(
  ...
  test(TestHelpers::test_creation<std::unique_ptr<TimeDependence<3>>>(
       ...
       initial_time, f_of_t_name, min_radius, max_radius,
       {{-0.01, 0.02, 0.01}});
