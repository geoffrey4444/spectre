...
void test_factory_and_semantics() {
  ...
  CHECK(solution.teukolsky_wave().parity() == "even");
  CHECK(solution.teukolsky_wave().direction() == "ingoing");
...
void test_zero_amplitude_matches_wrapped_kerr_schild() {
  ...
  const KerrSchildTeukolsky solution{
      kerr_schild, KerrSchildTeukolsky::TeukolskyWaveOptions{0., 0, 20., 4.}};
...
void test_nonzero_teukolsky_perturbation() {
  ...
  const KerrSchildTeukolsky solution{
      kerr_schild,
      KerrSchildTeukolsky::TeukolskyWaveOptions{0.02, -1, 20., 4.}};
