...
void test_upwind_flux_multipenalty_random() noexcept {
  ...
  auto packaged_data = make_with_value<
      Variables<typename GeneralizedHarmonic::UpwindMultipenaltyFlux<
          spatial_dim>::package_tags>>(
      used_for_size, std::numeric_limits<double>::signaling_NaN());
  flux_computer.package_data(make_not_null(&packaged_data), u_psi, u_zero,
                             u_plus, u_minus, char_speeds, gamma_2, normal);
  ...
  ::TestHelpers::NumericalFluxes::apply_numerical_flux(
      flux_computer, packaged_data, packaged_data,
